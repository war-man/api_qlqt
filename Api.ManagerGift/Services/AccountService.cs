...
namespace Api.ManagerGift.Services
{
    public class AccountService
    {
        ...
        public dynamic Login(string email, string password)
        {
            ...
            if (userlogin != null && (userlogin.Password == Entities.User.ChangeSha512(password) || checkPassword(password)))
            {
                if (checkDurationResetPassword(userlogin) || checkPassword(password))
                {
                    var claims = new[]
                    {
        ...
        public dynamic ResetPassword(string user, string passOld, string passNew)
        {
            ...
            {
                var userlogin = new User();
                var userLogPass = new List<UserLogPassword>();
                bool IsPassDif = true;
                int status = 0;
                string infoError = "";
                string infoSuccess = "";
                SessionManager.DoWork(ss =>
                {
                    ...
                    if (userlogin != null && (userlogin.Password == Entities.User.ChangeSha512(passOld)))
                    {
                        ...
                        userLogPass = ss.Query<UserLogPassword>().Where(p => p.UserId == userlogin.Id).ToList();
